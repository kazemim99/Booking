# AsanRezerve Admin Panel - Create Admin User Guide

**Official guide for creating admin users for the AsanRezerve Admin Panel**
...
#### Step 1: Register User via AsanRezerve API

```bash
curl -X POST http://localhost:5000/api/v1/Users \
  -H "Content-Type: application/json" \
  -d '{
    "email": "admin@asanrezerve.com",
    "password": "AsanRezerve@Admin2025",
    "firstName": "AsanRezerve",
    "lastName": "Admin",
...
docker exec -it asanrezerve-postgres psql -U asanrezerve_admin -d asanrezerve_user_management

# Or connect directly if PostgreSQL is running locally
psql -U asanrezerve_admin -d asanrezerve_user_management -h localhost
```
...
UPDATE "Users"
SET "Role" = 'Admin'
WHERE "Email" = 'admin@asanrezerve.com';
...
--------------------------------------+------------------+-----------+----------+-------+----------
 <uuid-here>                          | admin@asanrezerve.com | AsanRezerve    | Admin    | Admin | true
```
...
```sql
-- Generate BCrypt hash for password "AsanRezerve@Admin2025"
-- You must use your backend's password hasher to generate this hash
-- Example: In .NET use BCrypt.HashPassword("AsanRezerve@Admin2025")
...
string password = "AsanRezerve@Admin2025";
...
async function hashPassword() {
  const hash = await bcrypt.hash('AsanRezerve@Admin2025', 11);
  console.log(hash);
}
...
curl -X POST http://localhost:5000/api/v1/Auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@asanrezerve.com","password":"AsanRezerve@Admin2025","rememberMe":false}' \
...
# Test connection
docker exec -it asanrezerve-postgres psql -U asanrezerve_admin -d asanrezerve_user_management -c "SELECT 1;"
```
...
# Or manually
cd src/APIGateway/AsanRezerve.Gateway && dotnet run &
cd src/UserManagement/AsanRezerve.UserManagement.API && dotnet run &
```
...
**Direct Connection:**
```bash
psql -U asanrezerve_admin -d asanrezerve_user_management -h localhost -p 5432
```

**Connection String:**
```
Host=localhost;Port=5432;Database=asanrezerve_user_management;Username=asanrezerve_admin;Password=YourPostgresPassword123!
```
...
-- Find user by email
SELECT * FROM "Users" WHERE "Email" = 'admin@asanrezerve.com';
...
1. **Check Logs:**
   - Gateway logs: `docker logs asanrezerve-gateway`
   - UserManagement API logs: `docker logs asanrezerve-usermanagement-api`
