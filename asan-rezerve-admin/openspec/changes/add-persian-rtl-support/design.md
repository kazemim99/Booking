...
## Context
The asanrezerve-admin application currently only supports English with left-to-right (LTR) text direction. The asanrezerve-frontend application already has a complete Persian RTL implementation that includes:
...
This design leverages the proven patterns from asanrezerve-frontend while adapting them for the admin panel's unique requirements (Ant Design Vue instead of custom components).
...
- Maintain code consistency with asanrezerve-frontend patterns
...
**Why**: Industry-standard i18n library for Vue 3, used successfully in asanrezerve-frontend
...
**Pattern**: Mirror asanrezerve-frontend's locale store architecture
```typescript
{
  currentLocale: 'fa' | 'en',
  direction: 'rtl' | 'ltr',
...
**Persistence**: Save preference to localStorage key: `asanrezerve_admin_locale`
