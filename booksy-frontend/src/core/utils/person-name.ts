/**
 * What counts as a person's real name.
 *
...
/** The name parts a signed-in user carries (the auth store's `User`). */
...
/**
 * What a header calls the signed-in person: a real first name for the button and a real full name for the menu,
 * each null when there is none — a phone sign-up that skipped the name is «مشتری 9384444636», not a name.
 */
export function userNames(user?: NamedUser | null): { first: string | null; full: string | null } {
  if (!user) return { first: null, full: null }
  const full = realNameOrNull(user.firstName, user.lastName) ?? personNameOrNull(user.fullName)
