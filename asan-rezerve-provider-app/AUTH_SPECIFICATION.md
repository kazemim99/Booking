...
**Status:** Source-of-truth specification for implementing authentication in the new Flutter `asanrezerve-provider-app`.
**Reference implementation:** Vue web app `asanrezerve-frontend` (production) + `AsanRezerve.Host` backend (UserManagement + ServiceCatalog contexts).
...
### 1.1 Vue frontend (`asanrezerve-frontend/src`)
...
| Phone number value object (normalization) | `Core/AsanRezerve.Core.Domain/ValueObjects/PhoneNumber.cs` |
...
`firstName`/`lastName`/`email` are **optional** everywhere in the provider auth path (backend supplies defaults: firstName `"ارائه‌دهنده"`, lastName = national number, email = `{national}@asanrezerve.provider`).
...
- HS256 signing; issuer `AsanRezerve`, audience `AsanRezerve.Users`; `ClockSkew = 0`.
...
**State management:** follow the customer app's existing Bloc pattern (per CLAUDE.md, `asanrezerve-customer-app` uses Bloc/Cubit; codegen broken → manual JSON + manual `get_it` DI). Provider auth `Bloc` must expose the explicit states listed in §6.
...
|---|---|---|---|
| G-1 | Entire provider auth flow absent in Flutter (`asanrezerve-provider-app` not created) | **Critical** | This spec is the build plan. |
...
`AsanRezerve.Host` (`OTP_SANDBOX_CODE=123456`), first through the logic stack and
...
- **E-1 phone invariant is real.** `countryCode:"IR"` works; `"+98"` returns 200
  ...
  complete-auth. (⚠️ `asanrezerve-customer-app` defaults to `"+98"` and therefore
...
| T12 | Response envelope `{success,data,…}` | `core/api/client/api-response.ts`; `http-client.ts` · `normalizeResponse` | `Infrastructure/AsanRezerve.API/Middleware/ApiResponseMiddleware.cs` | Confirmed |
