# AsanRezerve Provider — Design System Specification (v2, normative)

**Every new AsanRezerve Provider screen MUST follow this spec.** It supersedes the v1
observational audit.
...
3. **AsanRezerve provider app** current theme (`lib/config/theme/app_tokens.dart`,
...
Figma and production code disagree, with the ruling for AsanRezerve.
...
| Token | Value | Usage |
|---|---|---|
| `border` | `#EBEEF3` | Resting input border, tab divider, card border (AsanRezerve), price container fill |
...
| `dialogBarrier` | `#000000` 14% (`0x24`) | Dialog barrier (AsanRezerve token, keep) |
| `sheetBarrier` | `#000000` 28% (`0x47`) | Bottom-sheet barrier (AsanRezerve token, keep) |
...
- Dark mode: NOT specified (ColiRide's dark palette is mostly a copy of light —
  treat as unimplemented; AsanRezerve is light-only until a real dark spec exists).

## 2. Typography

Font: **Vazir** (AsanRezerve standard, Persian-first). ColiRide uses commercial
...
| Rule | Value |
|---|---|
| Screen gutter | **16** (Figma hub; AsanRezerve `AppSpacing.md`). Production menu uses 20 — ruled: AsanRezerve stays 16. |
...
or filled state; count badges per §5.2. AsanRezerve tabs: Today / Calendar / Clients /
Profile.
...
- **Motion** (AsanRezerve tokens, not from ColiRide — see §9): fast 180ms / medium
...
2. **Motion**: ColiRide defines no motion tokens; Figma prototype transitions
   unreadable via MCP. AsanRezerve's `AppMotion` (180/250 easeOutCubic) is our own
   convention — normative for AsanRezerve, unverified against ColiRide.
...
7. **Divergences ruled for AsanRezerve**:
   - Save/Cancel: Figma stacks; production pairs in a Row → both allowed (§5.6).
   - Screen gutter: Figma 16 vs production menu 20 → AsanRezerve uses 16.
