...
namespace AsanRezerve.Infrastructure.Core.EventBus;
...
public sealed class CapIntegrationEventPublisher : IIntegrationEventPublisher
{
    ...
    /// Convention: namespace.eventname (e.g., asanrezerve.servicecatalog.providerregistered)
    /// </summary>
    private static string GetTopicName(Type eventType)
    {
        ...
        // Get bounded context name (e.g., ServiceCatalog from AsanRezerve.ServiceCatalog.Application.IntegrationEvents)
        var contextName = namespaceParts.Length > 1 ? namespaceParts[1].ToLowerInvariant() : "asanrezerve";

        // Remove "IntegrationEvent" suffix from event name
        var eventName = eventType.Name.Replace("IntegrationEvent", "").ToLowerInvariant();

        return $"asanrezerve.{contextName}.{eventName}";
    }
