...
namespace AsanRezerve.Core.Domain.Abstractions.Entities.Specifications;
...
public sealed class OrderExpression<T>
{
    ...
    public bool IsSubsequentOrdering { get; }

    public OrderExpression(
        Expression<Func<T, object>> keySelector,
        OrderDirection direction,
        bool isSubsequentOrdering = false)
    {
        KeySelector = keySelector ?? throw new ArgumentNullException(nameof(keySelector));
        Direction = direction;
        IsSubsequentOrdering = isSubsequentOrdering;
    }
}
