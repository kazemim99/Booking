...
namespace Booksy.ServiceCatalog.Domain.Specifications
{
    ...
    {
        public static Expression<Func<Aggregates.Provider, bool>> WithinRadius(double latitude, double longitude, double radiusKm)
        {
        ...
        public static Expression<Func<Aggregates.Provider, bool>> InCity(string city)
        {
            return provider => provider.Address.City.ToLower() == city.ToLower();
        }

        public static Expression<Func<Aggregates.Provider, bool>> InState(string state)
        {
            return provider => provider.Address.State.ToLower() == state.ToLower();
        }

        public static Expression<Func<Aggregates.Provider, bool>> InCountry(string country)
        {
            return provider => provider.Address.Country.ToLower() == country.ToLower();
        }

        public static Expression<Func<Aggregates.Provider, bool>> OffersMobileServices()
        {
