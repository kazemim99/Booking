...
namespace AsanRezerve.ServiceCatalog.Infrastructure.Persistence.Seeders
{
    ...
    public sealed class StaffSeeder : ISeeder
    {
        ...
        private static readonly Dictionary<string, string[]> StaffByProviderEmail = new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["shahriar.barber@asanrezerve.ir"] = new[] { "شهریار احمدی", "مهدی نوروزی", "یاسین قربانی" },

            // سالن زیبایی نگین مغان — women's beauty salon
            ["negin.moghan@asanrezerve.ir"] = new[] { "نگین رستمی", "سمیرا اصلانی", "الهام پیرزاد" },

            // ناخن‌کده آرام — nail studio
            ["aram.nails@asanrezerve.ir"] = new[] { "آرام موسوی", "فاطمه نصیری" },

            // اسپا و ماساژ آرامش — spa
            ["aramesh.spa@asanrezerve.ir"] = new[] { "سحر قاسمی", "مینا دلیری", "حامد شکوری" },

            // ماساژ درمانی سلامت — massage therapy
            ["salamat.massage@asanrezerve.ir"] = new[] { "بابک نجفی", "رقیه ولیزاده" },

            // باشگاه بدنسازی پارس — gym
            ["pars.gym@asanrezerve.ir"] = new[] { "رضا علیزاده", "سینا محمدپور", "نگار بهرامی" },

            // خانه یوگا مهر — yoga house
            ["mehr.yoga@asanrezerve.ir"] = new[] { "مهسا کریمی", "آیدا شریفی" },

            // کلینیک پوست و مو مغان — skin & hair clinic
            ["moghan.clinic@asanrezerve.ir"] = new[] { "دکتر لیلا حسینی", "دکتر فرزاد عبدی", "نسترن رحیمی" },

            // دندانپزشکی لبخند — dental
            ["labkhand.dental@asanrezerve.ir"] = new[] { "دکتر امیر صادقی", "دکتر سولماز بابایی" },

            // فیزیوتراپی توان — physiotherapy
            ["tavan.physio@asanrezerve.ir"] = new[] { "سعید زارعی", "محسن اکبری" },

            // سالن زیبایی گلستان — beauty salon
            ["golestan.beauty@asanrezerve.ir"] = new[] { "گلناز یوسفی", "پریسا خدایی", "شیدا مرادی" },

            // آرایشگاه مردانه آریا — men's barbershop
            ["arya.barber@asanrezerve.ir"] = new[] { "آرش مرادی", "کامران سلیمانی" },
        };
