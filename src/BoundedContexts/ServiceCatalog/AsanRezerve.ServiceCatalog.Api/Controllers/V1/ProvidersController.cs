...
using AsanRezerve.ServiceCatalog.Api.Models.Requests.Extenstions;
using AsanRezerve.ServiceCatalog.Api.Models.Responses;
using AsanRezerve.ServiceCatalog.API.Models.Requests;
using AsanRezerve.ServiceCatalog.API.Models.Responses;
using AsanRezerve.ServiceCatalog.Application.Commands.Provider.ActivateProvider;
using AsanRezerve.ServiceCatalog.Application.Commands.Provider.AddStaffToProvider;
using AsanRezerve.ServiceCatalog.Application.Commands.Provider.CompleteProviderRegistration;
using AsanRezerve.ServiceCatalog.Application.Commands.Provider.CreateProviderDraft;
using AsanRezerve.ServiceCatalog.Application.Commands.Provider.DeactivateProviderStaff;
using AsanRezerve.ServiceCatalog.Application.Commands.Provider.RegisterProvider;
using AsanRezerve.ServiceCatalog.Application.Commands.Provider.RegisterProviderFull;
...
using AsanRezerve.ServiceCatalog.Application.Queries.Provider.GetProviderClients;
using AsanRezerve.ServiceCatalog.Application.Queries.Provider.GetDraftProvider;
...
using AsanRezerve.ServiceCatalog.Application.Queries.Provider.GetProvidersByStatus;
using AsanRezerve.ServiceCatalog.Application.Queries.Provider.GetProviderStaff;
...
using ServiceDto = AsanRezerve.ServiceCatalog.Application.Commands.Provider.RegisterProviderFull.ServiceDto;
using TimeSlotDto = AsanRezerve.ServiceCatalog.Application.Commands.Provider.Registration.TimeSlotDto;
using AsanRezerve.ServiceCatalog.Application.Queries.Provider.GetProviderAvailabilitySummary;
...
public class ProvidersController : ControllerBase
{
    ...
    // POST /providers/organizations and POST /providers/individuals were removed with the
    // provider-hierarchy model. They existed to choose which KIND of provider to create --
    // an Organization that could have staff, or a standalone Individual. There is one kind
    // now: a salon. Registration goes through the draft flow below (POST /providers/draft
    // and the step endpoints), and everyone who works at a salon is a membership of it.
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var response = result.Map(searchResult => new ProviderSearchResponse
        {
            ...
            StaffMemberCount = searchResult.StaffMemberCount
        });
    ...
    private static ProviderSummaryResponse MapToProviderSummaryResponse(
        AsanRezerve.ServiceCatalog.Application.Queries.Provider.GetProvidersByStatus.ProviderListViewModel provider)
    {
