...
Located at: `src/Infrastructure/AsanRezerve.API/Middleware/ClientRateLimitResolver.cs`
...
Located at: `src/Infrastructure/AsanRezerve.API/Middleware/CustomRateLimitMiddleware.cs`
...
3. Check logs for rate limiting messages:
   ```bash
   grep "Rate limit exceeded" logs/asanrezerve-servicecatalog-*.txt
   ```
