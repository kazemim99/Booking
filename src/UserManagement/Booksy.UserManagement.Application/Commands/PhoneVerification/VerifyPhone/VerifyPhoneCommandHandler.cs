    ...
    public sealed class VerifyPhoneCommandHandler : ICommandHandler<VerifyPhoneCommand, VerifyPhoneResult>
    {
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    return Result<VerifyPhoneResult>.Failure("Verification not found");
                }

                // Check if blocked
                if (verification.IsBlocked())
                {
                    return Result<VerifyPhoneResult>.Success(
                        new VerifyPhoneResult(
                            false,
            ...
            catch (InvalidOperationException ex)
            {
                ...
                _logger.LogWarning(ex, "Verification validation failed for VerificationId: {VerificationId}", command.VerificationId);
                return Result<VerifyPhoneResult>.Failure(ex.Message);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to verify phone for VerificationId: {VerificationId}", command.VerificationId);
                return Result<VerifyPhoneResult>.Failure("An error occurred while verifying. Please try again.");
            }
        }
    }
}
