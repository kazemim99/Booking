...
> **نکته معماری**: Backend یک **modular monolith** است — یک پروسه واحد (`AsanRezerve.Host` / پورت 5000) که هر دو bounded context (UserManagement و ServiceCatalog) را در خودش دارد. دیگر Gateway جداگانه یا سرویس‌های جداگانه روی پورت‌های 5001/5002 وجود ندارد.
...
فقط **یک پروژه** (`AsanRezerve.Host`) کافی است:
...
یا در Visual Studio: پروژه `AsanRezerve.Host` را به عنوان Startup Project انتخاب کنید، پروفایل **http** را انتخاب کنید و F5 بزنید — باید `http://localhost:5000` و صفحه Swagger باز شود.
...
```bash
cd C:\Repos\Booking\asanrezerve-customer-app
flutter create --platforms=web .   # فقط بار اول لازم است
flutter run -d chrome --web-port=5173
```
پورت `5173` را عمداً انتخاب کنید — Backend از قبل این پورت را در `Cors:AllowedOrigins` مجاز کرده است (`src/Host/AsanRezerve.Host/appsettings.json`)، پس نیازی به تغییر تنظیمات CORS نیست.
...
اگر روی وب اجرا می‌کنید و خطای CORS دیدید، مطمئن شوید با `--web-port=5173` اجرا کرده‌اید (یا یکی از پورت‌های مجاز دیگر: `3000`, `3001`, `7002` — لیست کامل در `src/Host/AsanRezerve.Host/appsettings.json` زیر `Cors:AllowedOrigins`).

### خطا: "502 Bad Gateway"

دیگر معنایی ندارد — این خطا مخصوص معماری قدیمی Gateway بود. اگر `AsanRezerve.Host` سالم است، این خطا رخ نمی‌دهد.
...
4. فایروال را برای پورت 5000 باز کنید:
   ...
   New-NetFirewallRule -DisplayName "AsanRezerve Backend" -Direction Inbound -LocalPort 5000 -Protocol TCP -Action Allow -Profile Private,Public
   ```
...
- تنظیمات CORS: `../src/Host/AsanRezerve.Host/appsettings.json` (`Cors:AllowedOrigins`)
