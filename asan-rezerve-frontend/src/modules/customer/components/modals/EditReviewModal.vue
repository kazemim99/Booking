<template>
  <BaseModal :is-open="isOpen" @close="emit('close')" title="ویرایش نظر" size="md">
    <ReviewForm
      :initial="{ rating: review.rating, comment: review.text ?? '', dimensions: review.dimensions }"
...
/**
 * Edit one of the customer's reviews, with the same form used to write one — the four optional dimensions, and a
 * 10–2000 character comment. (This modal used to carry its own form with a 500-character limit and no dimensions,
...
function save(input: ReviewInput) {
  emit('save', props.review.id, { rating: input.rating, text: input.comment, dimensions: input.dimensions })
}
</script>
