...
/// the app offers it twice: a row on the More page, always there, and a one-time card on Home — a salon gets no SMS
/// for a new booking request, so push is its only way to hear about one while the app is closed.
...
void main() {
  ...
  group('More row', () {
    ...
    testWidgets('the More page frames the row in its own section, and shows no empty section without push',
        (tester) async {
      Widget framed(PushPermissionCubit c) => PushEnableTile(
            cubit: c,
            frame: (context, row) => Column(children: [const Text('section-header'), row]),
          );

      await pump(tester, framed, _FakeSettings(PushStatus.notAsked));
      expect(find.text('section-header'), findsOneWidget);

      await pump(tester, framed, _FakeSettings(PushStatus.unavailable));
