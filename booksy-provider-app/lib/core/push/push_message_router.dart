...
import '../../config/routes/app_router.dart' show Routes;
...
class PushMessageRouter {
  ...
  static Future<void> attach({
    ...
    void open(Map<String, dynamic> data) {
      inbox.refreshCount();
      router.push(pushOpenRoute(data));
    }
    ...
    // Arrived while the salon is using the app. Neither Android nor the browser shows a banner for the app in front, so
    // the app shows a snackbar — never taking the screen the person is using — with a way to open what it is about.
