...
/// The customer's inbox AND the bell's badge — one cubit for both, so the number on the bell and the list it
/// opens cannot disagree. Reads are optimistic, and put back exactly as they were if the server refuses.
class InboxCubit extends Cubit<InboxState> {
  ...
  /// The badge on its own. A failure leaves it where it was (zero for a fresh cubit) — a guessed number is
  /// worse than none.
  ...
  Future<bool> markRead(String id) async {
    ...
    // Already read, or not on this page: nothing to change, nothing to tell the server. Decrementing for a row
    // that was already read is how the badge drifts below the truth.
    ...
    emit(state.copyWith(items: items, unreadCount: state.unreadCount > 0 ? state.unreadCount - 1 : 0));
