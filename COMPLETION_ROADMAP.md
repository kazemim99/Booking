...
- [x] **Wired the keystone E2E into CI**: `deploy-staging.yml` now has an `e2e-keystone` job (ephemeral Postgres+Redis services) that boots the real `Booksy.Host` with sandbox auth (`OTP_SANDBOX_CODE`, `Rahyab__SandboxMode=true`, `Kavenegar__Enabled=false`), waits for `/health`, then runs `tests/e2e/keystone-booking-flow.sh`. `build-and-push` now gates on `needs: [test, e2e-keystone]`, so a broken core flow blocks deploy. *(Optional follow-up: also port it to an xUnit `WebApplicationFactory<Program>` integration test with Testcontainers.)*
...
- [ ] Harden the Flutter customer app ↔ backend contract (features scaffolded: auth/booking/search/profile/home).
