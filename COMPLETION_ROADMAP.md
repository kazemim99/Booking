...
- [x] **Retargeted `Booksy.ServiceCatalog.IntegrationTests` to `Booksy.Host`** (2026-08-24). It had been booting `WebApplicationFactory<Startup>` resolved from `Booksy.API` — the retired per-service host — via a one-line alias in `GlobalUsing.cs` (`global using Startup = Booksy.Host.HostEntryPoint;`), the same marker-type trick `Booksy.Host.CompositionTests` already used. Every assertion in ~100 files had been made against a host that no longer exists post-monolith.
