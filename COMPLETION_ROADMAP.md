# AsanRezerve — Completion Roadmap

> Tracked plan to take AsanRezerve from "MVP with uneven implementation" to a launchable
...
- [x] Backend is a single modular-monolith host (`AsanRezerve.Host`), runs E2E, 3 schemas in one DB, CAP in-process.
...
- [x] Frontends reskinned (Coliride) + shared `@asanrezerve/tokens`; both build green.
...
- [x] **Wired the keystone E2E into CI — both pipelines**: an `e2e-keystone` job (ephemeral Postgres+Redis) boots the real `AsanRezerve.Host` with sandbox auth (`OTP_SANDBOX_CODE`, single `Sms:SandboxMode=true`), waits for `/health`, then runs `tests/e2e/keystone-booking-flow.sh`. Added to **`deploy-staging.yml`** (develop → `build-and-push needs: [test, e2e-keystone]`) **and `deploy.yml`** (master → production: `build-api`/`build-frontend` need `[test, e2e-keystone]`). Also **re-enabled the unit-test run in `deploy.yml`** (it had been commented out — production was deploying on a successful compile alone). A broken core flow now blocks both the staging and production deploys. *(Optional follow-up: also port it to an xUnit `WebApplicationFactory<Program>` integration test with Testcontainers; retarget the per-service integration test projects to `AsanRezerve.Host` and re-enable the integration-test step.)*
- [x] **Retargeted `AsanRezerve.ServiceCatalog.IntegrationTests` to `AsanRezerve.Host`** (2026-08-24). It had been booting `WebApplicationFactory<Startup>` resolved from `AsanRezerve.API` — the retired per-service host — via a one-line alias in `GlobalUsing.cs` (`global using Startup = AsanRezerve.Host.HostEntryPoint;`), the same marker-type trick `AsanRezerve.Host.CompositionTests` already used. Every assertion in ~100 files had been made against a host that no longer exists post-monolith.
  - **Fixed the suite from unrunnable to runnable first**: `TestWebApplicationFactory` started one Postgres *container* per test class (xUnit runs classes in parallel) — 102 concurrent containers on a full run, which exhausts memory locally and could never run in CI. `PostgresTestContainerFixture` now shares one container per test *process* and gives each class its own database on it; isolation is unchanged, container count 102 → 1.
  - **Fixed the config-ordering bug the retarget then exposed**: `CAP`/other services read `ConnectionStrings:DefaultConnection` during `Program.cs` service registration, before `ConfigureAppConfiguration` applies — every registration-time consumer kept the Host's own dev connection string (port 54321) instead of the test container's. Injected via `IWebHostBuilder.UseSetting` instead, which lands before the app builder reads configuration. This alone cleared entire test classes (e.g. `CategoriesControllerTests` 11→0, `ProviderCategoryPersistenceTests` 10→0).
  - **Triaged, not just retargeted**: found and fixed real production defects the retarget surfaced for the first time (a Host composing both contexts reaches code paths the old per-service host never could) — 7 IDOR gaps in `ProviderSettingsController` (every GET action missing the ownership check every PUT/POST already had), a partial-update handler that nulled `BusinessDescription` on omission, a `GetBusinessInfo`-family field-mapping bug (`AddressLine1` returning `FormattedAddress`), a gallery-reorder cache-invalidation gap (ADR-005-adjacent), and several stale tests asserting fields/rules that don't exist on the current contract (documented in `openspec/changes/FOLLOW-UPS.md` so the pattern is recognizable next time, not just patched once).
  - **Net result**: `ProviderSettingsControllerTests`/`ProviderSettingsTests` 15→0 failures (42/42); `GalleryManagementTests` 13→0 (31/31); the 5 fully-bound Reqnroll feature files (`CreateBooking`, `CancelBooking`, `RescheduleBooking` content-audited and domain-verified); unit+architecture suite 820/820 throughout.
  - **What's still open, by design, not oversight**: `openspec/changes/REQNROLL-COVERAGE-GAP.md` records that 707 of 739 Reqnroll scenarios are blocked by unbound step definitions — a specification-coverage backlog, not failing tests (implement over weeks, or quarantine behind a filtered tag — a scope decision, not a technical one). Separately, `ProcessPayment.feature`/`RefundPayment.feature`/the ZarinPal+Behpardakht feature family are **credentials-blocked**: their step definitions construct `Mock<IZarinPalService>`/`Mock<IBehpardakhtService>` but never register them with the test DI container, so those scenarios make genuine outbound calls to the real gateway APIs and fail on the same placeholder `MerchantId` `checkout-release-gates` already tracks. The general CI workflow (`dotnet.yml`) still does not run this suite — deliberately not re-enabled wholesale, since a large known-blocked fraction would make CI red by design and retrain people to ignore it; wiring in only the runnable subset (or a `@wip`-style tag filter) is a follow-up, not done here.
...
- [x] **Deleted the dead `src/BoundedContexts/Booking` context** — no source, no `.csproj`, not in the solution (only stale `bin/obj` artifacts); plus a stray `src/BoundedContexts/obj`. Bookings live in ServiceCatalog. `AsanRezerve.Gateway` was already gone (only a `logs/gateway` dir remains).
...
- [ ] Containerize: one real `docker build` of `src/Host/AsanRezerve.Host/Dockerfile` (never verified).

### Epic 3.4 — Observability
- [x] **Booking-funnel metrics added**: `BookingMetrics` (static `Meter` `AsanRezerve.ServiceCatalog.Bookings`) emits counters `asanrezerve.bookings.created / confirmed / completed / cancelled / noshow`, incremented in the 5 booking command handlers right after their successful commit. OpenTelemetry already collects them via `AddMeter("AsanRezerve.*")` — exported wherever `PROMETHEUS_ENDPOINT`/`OTLP_ENDPOINT` is configured (deployment-gated). Seq logging + health checks were already wired.
...
- [ ] Promote `@asanrezerve/tokens` to a real workspace dependency.
