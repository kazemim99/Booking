...
using Xunit;
using DomainProvider = AsanRezerve.ServiceCatalog.Domain.Aggregates.Provider;
...
public class ProviderDepositPolicyTests : ServiceCatalogIntegrationTestBase
{
    public ProviderDepositPolicyTests(AsanRezerveHostFactory factory)
        : base(factory) { }
    ...
    public async Task The_command_is_marked_for_provider_ownership_enforcement()
    {
        ...
        typeof(UpdateBookingPreferencesCommand)
            .Should().Implement<AsanRezerve.Core.Application.Authorization.IRequireProviderOwnership>();
    }
