...
{
    public StaffMembershipNotificationTests(AsanRezerveHostFactory factory) : base(factory)
    {
    }
    ...
    private async Task<(Guid PersonId, string Phone)> CreateRealPersonAsync()
    {
        ...
        var user = global::AsanRezerve.UserManagement.Domain.Aggregates.User.RegisterWithPhone(
            Core.Domain.ValueObjects.Email.Create($"staff-{Guid.NewGuid():N}@test.com"),
            Core.Domain.ValueObjects.PhoneNumber.From(phone),
            global::AsanRezerve.UserManagement.Domain.Entities.UserProfile.Create("زهرا", "محمدی"),
            global::AsanRezerve.UserManagement.Domain.Enums.UserType.Provider);
