...
using AsanRezerve.Tests.Commons;
...
/// <summary>
/// Base class for every integration test in this project that needs full API and database access.
///
/// Generic only over <c>TDbContext</c> now — not over the factory or the entry point too
/// (docs/TEST_ARCHITECTURE_AUDIT.md Phase 2 slice 4). There is exactly one factory type
/// (<see cref="AsanRezerveHostFactory"/>) and exactly one entry point (<c>Startup</c>, aliased in
/// GlobalUsing.cs) in this project, so threading them through as type parameters bought nothing
/// but two more type arguments at every call site; <c>TDbContext</c> earns its keep because
/// ServiceCatalog and UserManagement tests each want their own typed <c>DbContext</c> property.
///
/// <para>Deliberately does NOT declare <c>IClassFixture&lt;AsanRezerveHostFactory&gt;</c>: every leaf
/// test class instead joins <see cref="AsanRezerveHostTestCollection"/> via
/// <c>[Collection(AsanRezerveHostTestCollection.Name)]</c>, which is what actually supplies the one
/// shared factory instance — xUnit does not honour a <c>[Collection]</c> attribute inherited from a
/// base class, only one declared directly on the concrete test class.</para>
/// </summary>
public abstract class IntegrationTestBase<TDbContext> : IAsyncLifetime
    where TDbContext : DbContext
{
    public readonly AsanRezerveHostFactory Factory;
    ...
    public IntegrationTestBase(AsanRezerveHostFactory factory)
    {
        Factory = factory;
