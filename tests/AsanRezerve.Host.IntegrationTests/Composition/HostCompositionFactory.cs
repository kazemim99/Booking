...
/// <summary>
/// Boots the real <c>AsanRezerve.Host</c> — the same top-level Program the container runs — against
...
// AsanRezerve.Host and AsanRezerve.UserManagement.API declare a global `Program` (CS0433 if named
// directly). Any type from AsanRezerve.Host serves; the adapter under test is the clearest.
public sealed class HostCompositionFactory
    : WebApplicationFactory<AsanRezerve.Host.Composition.InProcessProviderInfoService>, IAsyncLifetime
{
