...
namespace AsanRezerve.UserManagement.Application.UnitTests.Domain;

/// <summary>
/// Every AsanRezerve account is created by OTP, so <see cref="User.RegisterWithPhone"/> decides the
...
public class PhoneRegistrationStatusTests
{
    private static User PhoneRegistered(UserType type = UserType.Provider) =>
        User.RegisterWithPhone(
            Email.Create("s@asanrezerve.test"),
