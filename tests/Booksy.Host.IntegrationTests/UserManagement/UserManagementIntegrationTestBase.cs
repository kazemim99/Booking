...
namespace Booksy.Host.IntegrationTests.UserManagement;
...
/// <see cref="BooksyHostTestCollection"/> via <c>[Collection(BooksyHostTestCollection.Name)]</c> —
/// the SAME collection ServiceCatalog tests join, since both now share one
/// <see cref="BooksyHostFactory"/> (docs/TEST_ARCHITECTURE_AUDIT.md Phase 2 slice 4).
/// </summary>
public abstract class UserManagementIntegrationTestBase
    : IntegrationTestBase<UserManagementDbContext>
{
    public UserManagementIntegrationTestBase(BooksyHostFactory factory)
        : base(factory)
    {
    }
