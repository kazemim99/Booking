...
/// (<see cref="BooksyHostFactory"/>) and exactly one entry point (<c>Startup</c>, aliased in
...
///
/// <para>Deliberately does NOT declare <c>IClassFixture&lt;BooksyHostFactory&gt;</c>: every leaf
/// test class instead joins <see cref="BooksyHostTestCollection"/> via
/// <c>[Collection(BooksyHostTestCollection.Name)]</c>, which is what actually supplies the one
...
{
    public readonly BooksyHostFactory Factory;
    ...
    public IntegrationTestBase(BooksyHostFactory factory)
    {
        Factory = factory;
