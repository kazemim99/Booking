...
Defines how a person relates to an organization (salon): one `Person` account per phone number
can hold memberships in zero, one, or many organizations, each membership carrying its own
organization-scoped roles, lifecycle status, and (for service-providing staff) a `StaffProfile`.
This capability replaced the earlier `Provider.ParentProviderId` sub-provider hierarchy — created
by archiving change `refactor-identity-and-membership` (2026-09-11).
