# booking-data-integrity Specification

## Purpose
Keeps the `Bookings` table's read paths (my-bookings, provider dashboard, conflict checks) index-supported and free of orphaned/duplicated financial state, so booking queries stay fast and a reschedule never leaves a booking without its own payment record.
## Requirements
