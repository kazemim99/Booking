...
## Purpose
Defines how the repository organizes its markdown documentation so a reader can tell, from a file's location alone, whether it describes current behavior — and keeps `CLAUDE.md` accurate about the testing setup. Created by archiving change consolidate-project-documentation.
...
- **THEN** it finds a section covering the Playwright E2E suite (`asanrezerve-frontend/e2e/`) and the xUnit integration tests against the composed host (`tests/AsanRezerve.ServiceCatalog.IntegrationTests/`)
