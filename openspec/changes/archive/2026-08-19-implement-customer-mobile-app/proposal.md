...
> ## ⚠ OBSOLETE — archived 2026-08-19 without promoting its specs
>
> **Never finished as a proposal** (its own README says "60% complete — 3 of 5 specs done", 8 of 105 tasks
> ticked) and **superseded twice** by work that shipped the app it was planning:
>
> - `customer-app-ux-redesign` (2026-07) delivered the whole application — M3 token theme, a 19-component
>   library in `lib/core/widgets/`, a go_router stateful shell, auth, home, explore, provider detail, the
>   stepped booking flow, appointments, and profile, with 41 tests.
> - `unify-customer-app-with-provider-design` (archived 2026-07-23) then converged its visual language onto
>   the provider/Coliride design and produced the `customer-app-*` / `customer-*-journey` capabilities.
>
> **Its tasks now contradict the shipped app**, which is the clearest signal it is obsolete: task 1.1.2 adds
> `pull_to_refresh` (removed by `customer-app-ux-redesign` §1.6); task 1.2.3 builds components under
> `lib/shared/components/` (no `lib/shared/` exists); task 1.2.2 mandates ScreenUtil sizing (`.sp` was
> deliberately stripped from all text styles in §9.2); the design section pins a `#1A365D` flat palette (the
> app's primary is `#3777BF`, matching the provider app); and task 1.1.3 records code generation as working
> (`CLAUDE.md` records `build_runner` as broken — new services use manual JSON and manual `get_it`).
>
> Its "Questions for Stakeholders" were all answered months ago: ZarinPal for payments, Neshan for maps,
> external-browser payment return.
>
> **Archived with `--skip-specs`.** Its five `customer-mobile-*` capabilities would have added a third
> parallel description of the customer app alongside `customer-app-*` and `customer-*-journey`. If any of its
> 80+ scenarios prove to cover something those specs miss, harvest them individually rather than promoting
> the capability.
