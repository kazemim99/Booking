...
Delta against `openspec/specs/notification-delivery/spec.md`.

Two changes. The delivery guarantees were written for customers and now cover providers and staff, who
receive notifications for the first time in this change. And a channel that cannot send must say so: the push
channel currently reports success without sending, which makes the delivery log untrustworthy precisely where
it is meant to be evidence.
