...
The per-notification contract: for every notification the system can send, who receives it, on which
channels, how urgent it is, whether the recipient may switch it off, and where tapping it leads.

This is the layer that makes the notification set reviewable. Without it, what the product sends is
discoverable only by reading handlers.
