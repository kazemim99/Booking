# Test Architecture Audit — AsanRezerve (2026-09-11)
...
(`AsanRezerve.ServiceCatalog.IntegrationTests`, 513 tests) running strictly serially because the fixture
...
1. **The UserManagement integration suite tests a host that does not exist in production.** It
   references only `AsanRezerve.UserManagement.API` (the retired per-service host) and boots that
   project's `Program`, not `AsanRezerve.Host`. The ServiceCatalog suite was retargeted in August; this one
...
|---|---------|--------|----------|
| 1 | Host booted **per test class** in all three DB projects (53 + 8 + 4 boots per FULL run), with seeding on | Time: the majority of the 8.2 min SC step; memory; 53 hosts appending to the same `logs/asanrezerve-host-*.txt` | `IntegrationTestBase : IClassFixture<TFactory>`; `Program.cs:269` `seed = … EnvironmentName.Contains("Test")`; `TestWebApplicationFactory` sets `UseEnvironment("Test")` |
...
| 3 | **UserManagement suite boots the retired per-service host** | Correctness of the gate: 42 tests validate a composition that never ships; cross-context in-process adapters (`InProcess*`) are absent from what they test | `AsanRezerve.UserManagement.IntegrationTests.csproj` references `AsanRezerve.UserManagement.API`, not `AsanRezerve.Host`; `UserManagementIntegrationTestBase` uses `Program` from that assembly |
...
|---|---|---|---|---|---|---|
| `AsanRezerve.Core.Domain.UnitTests` | unit | 44 | 0.45 s | 5.1 s | no | FA 6.12 |
| `AsanRezerve.Infrastructure.Core.UnitTests` | unit | 11 | 0.40 s | 4.6 s | no | FA 6.12 |
| `AsanRezerve.ServiceCatalog.Domain.UnitTests` | unit | 548 | 0.19 s | 4.4 s | no | exemplary: pure aggregate/VO tests |
| `AsanRezerve.ServiceCatalog.Application.UnitTests` | unit | 205 | 0.62 s | 16.3 s | no | NSubstitute; contains `UnitTest1.cs` placeholder |
| `AsanRezerve.UserManagement.Application.UnitTests` | unit | 44 | 3 s | 12.5 s | no | `OtpCodeTests` mutates env vars |
| `AsanRezerve.ArchitectureTests` | unit | 8 | 2 s | 18 s | no | builds EF model in-memory; NetArchTest |
| `AsanRezerve.Host.CompositionTests` | db | 21 (4 classes) | 45 s | 75.6 s | yes ×4, env `Staging` | only project already targeting `AsanRezerve.Host` correctly |
| `AsanRezerve.ServiceCatalog.IntegrationTests` | db | 513 (53 classes, 2 skipped) | 7 m 44 s | 492.6 s | yes ×53, env `Test` (seeds) | parallelism disabled; 8 unit-test files inside |
| `AsanRezerve.UserManagement.IntegrationTests` | db | 42 (8 classes) | 35 s | 66.5 s | yes ×6 (retired `UserManagement.API` host) + 2 raw-context classes | not retargeted to the monolith |
| `AsanRezerve.Tests.Commons` | shared | – | – | – | – | fixtures, factory, auth handler, 2 builders, AutoFixture |
| `AsanRezerve.Tests.Common` | **dead** | – | – | – | – | not in `.sln`; nested csproj path; kitchen-sink packages |
| `AsanRezerve.ServiceCatalog.UnitTests/` | **orphan dir** | 9 `[Fact]` never compiled | – | skipped | – | listed in `verify.ps1:162` and `verify.sh:77` |

Verify step time minus test time is MSBuild re-evaluation (no `--no-build`): 5–16 s per unit project,
~30 s per DB project.

Non-.NET: Vue `asanrezerve-frontend` (12 vitest spec files, 4 Playwright specs, 1 Cypress spec),
`asanrezerve-admin` (vitest), Flutter customer app (26 test files), provider app (44). Vue/Flutter steps
...
- **Infrastructure**: `ServiceCatalogTestWebApplicationFactory` (adds `FakePaymentGateway` + four
  ...
  (aliases `Startup` → `AsanRezerve.Host.HostEntryPoint`), `AssemblyInfo` (`DisableTestParallelization`).
...
1. **Container**: `PostgresTestContainerFixture` — one static `postgres:16-alpine` per test process
   (good; the earlier design started 102 containers), a fresh `CREATE DATABASE asanrezerve_test_<guid>` per
...
3. **Host startup** (`AsanRezerve.Host/Program.cs:269-276`): migrates **both** contexts again, seeds both
...
| L5 | **Template database**: migrate one `asanrezerve_template` per process, then `CREATE DATABASE … TEMPLATE asanrezerve_template` per fixture (≈ 100–300 ms) instead of running 24 migrations + CAP DDL per fixture | migrations per class | one migration per process | medium | CAP's storage initialiser still runs per host (idempotent); Postgres refuses `TEMPLATE` while another session is connected to the template — keep the template connection-free after migrating (`WITH (FORCE)` drop of stray connections, or `pg_terminate_backend`) |
...
| L11 | Retarget the UM suite to `AsanRezerve.Host` and merge the three DB projects (§4) | three assemblies, three container/host stacks | one | medium | mostly moves; namespaces change |
...
| 3 — UM on the real `AsanRezerve.Host` | UM 31 (uncontended FULL) | 42 UM | replaced the retired UM.API host |
| 4 — one project, `AsanRezerve.Host.IntegrationTests` | **111** (was 3 steps, ~296 contended) | **480** | one project, one `db:` step; 2 production defects found and fixed (register-provider validation ordering, a bare `InvalidOperationException` mapping to 500) |
| 5 — 2 collections in parallel | **94–100** | **480** | `AsanRezerveHostTestCollection` ∥ `HostCompositionCollection`; a pre-existing stress-test deadlock's `catch` clause fixed to match what it already claimed to tolerate |
...
2. **Does correctness depend on something only the real stack provides?** EF mapping and conventions,
   ...
   DTO shape) → **Integration test against `AsanRezerve.Host`.** One integration test per endpoint per
...
| `…/Unit/PaymentGatewayFactoryTests`, `FakeZarinPal*Tests` | new `AsanRezerve.Infrastructure.External.UnitTests` (also home for the orphaned `ZarinPalServiceTests`) | infrastructure adapters with `HttpMessageHandler` fakes |
...
`tests/AsanRezerve.Host.IntegrationTests` replaces `AsanRezerve.ServiceCatalog.IntegrationTests`,
`AsanRezerve.UserManagement.IntegrationTests` and `AsanRezerve.Host.CompositionTests`. There is one production
composition (`AsanRezerve.Host`); there is one integration project that boots it.

```
tests/
  AsanRezerve.Tests.Shared/                 (rename of AsanRezerve.Tests.Commons; NO Testcontainers, NO Mvc.Testing)
  ...
  AsanRezerve.Host.IntegrationTests/
    Infrastructure/
      Postgres/    PostgresServerFixture (static container + template DB), TestDatabase (clone/drop)
      Host/        AsanRezerveHostFactory (the ONE WebApplicationFactory<HostEntryPoint>), HostFixture (collection fixture)
...
`AsanRezerve.Tests.Shared` is referenced by unit projects too, so it must stay free of Testcontainers,
`Mvc.Testing` and EF providers. Delete `AsanRezerve.Tests.Common` outright.

### 4.2 Fixture hierarchy

| Level | Type | Lifetime | Responsibility |
|---|---|---|---|
| process | `PostgresServerFixture` (static, lazy, gated) | test run | start `postgres:16-alpine` once; create `asanrezerve_template`; migrate both contexts into it **once** using the host's own `DbContextOptions` (not hand-rolled — that is what produced the pending-model-changes discrepancy); disconnect |
| collection | `HostFixture : ICollectionFixture` | xUnit collection | `CREATE DATABASE <name> TEMPLATE asanrezerve_template`; boot one `AsanRezerveHostFactory` against it; expose `Services`, `Api`, `Fakes`, `Clock`; drop the database on dispose |
...
`AsanRezerveHostFactory : WebApplicationFactory<HostEntryPoint>`:

- `UseEnvironment("Testing")` and ship `src/Host/AsanRezerve.Host/appsettings.Testing.json` so the test
...
| Host log file shared across 53 hosts | `logs/asanrezerve-host-*.txt` under the test bin dir | xUnit logger provider; no file sink in `Testing` |
...
| **PR full** (new job in `dotnet.yml`) | every PR | `AsanRezerve.Host.IntegrationTests` via Testcontainers (Docker is on `ubuntu-latest`; no `services:` block needed), `--blame-hang-timeout 5m`, trx artefact; quarantined excluded | ≤ 8 min now, ≤ 5 min after Phase 2 | **disabled with a stale comment** — enable in Phase 1 |
...
- Rewrite `## Test suites` for the new layout (`AsanRezerve.Host.IntegrationTests`, collections, how to run
...
- **Retired hosts in the solution.** `AsanRezerve.UserManagement.API` and `AsanRezerve.ServiceCatalog.Api`
...
`dotnet test tests/AsanRezerve.ServiceCatalog.IntegrationTests --no-build --logger trx` on the audit machine
...
| 1.3 ⟂ | Explicit seed gate: `Database:SeedOnStartup` replaces `EnvironmentName.Contains("Test")` in `AsanRezerve.Host/Program.cs:269` and `AsanRezerve.ServiceCatalog.Api/Startup.cs:222`; default `true` when Development; `TestWebApplicationFactory` sets `false`; `HostCompositionFactory` can drop its `Staging` workaround | L3 — part of the measured 9 s per-class boot (§2.2); the share is unknown until this lands, so record the new per-class first-test time | the two entry points, both test factories, `appsettings.Development.json` (document the key) | Development still seeds by default; CI e2e jobs run as Development → unchanged | up to several seconds × 43 classes | P0 |
| 1.14 ⟂ | Quiet the hosts under test: `src/Host/AsanRezerve.Host/appsettings.Testing.json` (Serilog `MinimumLevel: Warning`, console sink only, no file sink) and `UseEnvironment("Testing")` in both factories; confirm the trx logger now writes a file | L13; problem #17 — 1.4 GB per run | Host appsettings, both factories, `verify.ps1` log tail | none | faster runs, readable logs, working trx | P0 |
| 1.4 ⟂ | Remove `services.BuildServiceProvider()` + `dbContext.Database.Migrate()` from `TestWebApplicationFactory.ConfigureServices` | L4; ASP0000; host migrates anyway | `tests/AsanRezerve.Tests.Commons/TestWebApplicationFactory.cs:138-141` | none | ~0.5–1 s × 53 | P0 |
| 1.5 ⟂ | Delete `tests/AsanRezerve.Tests.Common/` (dead), `SC.Application.UnitTests/UnitTest1.cs`; remove `tests/AsanRezerve.ServiceCatalog.UnitTests` from both verify scripts and move its one file (see 1.7) | problem #7 | those paths, `scripts/verify.*` | none | clarity | P0 |
| 1.6 ⟂ | Composition tests share one host: `[CollectionDefinition]` + `ICollectionFixture<HostCompositionFactory>` on the four classes | L9 | `tests/AsanRezerve.Host.CompositionTests/*.cs` | none | 45 s → ~15 s | P1 |
| 1.7 | New `tests/AsanRezerve.Infrastructure.External.UnitTests` (NSubstitute, FA pinned); move `SC.IntegrationTests/Unit/*` there or to the Application/Core unit projects per §3.2; move the orphaned `ZarinPalServiceTests` in (port Moq → NSubstitute while moving); add the project to the `.sln`, both verify scripts and the CI unit filter | problem #8 | new project, `AsanRezerve.sln`, `scripts/verify.*`, `.github/workflows/dotnet.yml` | `TokenServiceTests` tests a retired adapter — move now, delete with the adapter later | 8 files leave the Docker tier; unit gate gets ~40 more tests | P1 |
...
| # | Item | Why | Files / areas | Risk | Benefit | Priority |
|---|---|---|---|---|---|---|
| 2.1 | Create `tests/AsanRezerve.Host.IntegrationTests` with `Infrastructure/` per §4.1: `PostgresServerFixture` (container + **template database** migrated once with the host's real `DbContextOptions`), `TestDatabase` (clone/drop), `AsanRezerveHostFactory` (one factory, env `Testing`, `appsettings.Testing.json`), `HostFixture` collection fixture, `Collections.cs`, `xunit.runner.json` | L5, L6, L11; the core of the redesign | new project; `src/Host/AsanRezerve.Host/appsettings.Testing.json` | template + parallel clone edge cases (§2.3 L5) — cover with a fixture self-test | one boot per collection; migrations once | P0 |
...
| 2.12 | Delete `AsanRezerve.ServiceCatalog.IntegrationTests`, `AsanRezerve.UserManagement.IntegrationTests`, `AsanRezerve.Host.CompositionTests`; update `.sln`, both verify scripts, `dotnet.yml`, `CLAUDE.md`, `AGENTS.md`, `docs/KNOWLEDGE_MAP.md`; write `tests/README.md` | §7 | docs and scripts | none | | P0 (closes the phase) |
...
| 3.5 | Retire Cypress (one spec) in favour of Playwright; keep Playwright at 3–4 keystone journeys; promote it to a deploy gate only if it stays green for a month of nightlies | §3.2 | `asanrezerve-frontend/cypress`, `package.json`, `frontend-e2e.yml` | none | one browser stack | P2 |
